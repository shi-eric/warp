...
class TestBuildLibTargetSelection(unittest.TestCase):
    def test_windows_cross_build_rejects_cuda(self):
        output = io.StringIO()
        with (
            ...
            result = build_lib.main(
                ["--target-arch", "aarch64", "--no-use-libmathdx", "--no-standalone", "--no-verbose"]
            )

        self.assertEqual(result, 1)
        self.assertIn("currently supports CPU-only builds", output.getvalue())
