...
# Newton examples — compile time only (3 samples each)
newton_examples = {
    "labels": [
        "Robot UR10",
        "Robot Allegro",
...
def plot_comparison(ax, labels, main_vals, branch_vals, title, ylabel="Compile time (s)"):
    ...
    ax.set_ylim(bottom=0, top=1.3 * max(main_vals))
    ax.grid(axis="y", linewidth=0.5, alpha=0.7)
    ax.set_axisbelow(True)

    # Add speedup labels above the "after" bars — small font, offset above bar top
