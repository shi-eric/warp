...
- Untargeted CUDA graphs use the captured ``.cubin`` or ``.ptx`` files.
- A targeted CUDA export contains only the selected final ``.cubin`` or ``.ptx``
  binary for each module, without generated source, metadata cache files, LTO-IR,
  fatbins, or alternate-format fallbacks.
...
CUDA target selection happens on the producer during ``capture_save()``. An
untargeted save copies the captured binary without compilation. A targeted save
with ``target_arch=N`` produces CUBIN for ``sm_N`` by default, while
``target_arch=N, use_ptx=True`` produces final linked PTX for ``compute_N``.
Baseline PTX treats ``N`` as a minimum compute capability and can be
driver-JITed on a compatible newer GPU. ``a`` and ``f`` suffixes narrow that
compatibility; CUDA remains authoritative for suffixed PTX and CUBIN.
...
The ``.wrp`` file never contains generated source, a compiler recipe, or link
inputs. At load time, Python and standalone native consumers use the same
native module-loading path; the only guest-local compilation is for runtime
conditional helpers, not serialized user modules. Missing or incompatible final
artifacts cannot be regenerated by the guest.

### Compatibility policy

#### ``.wrp`` format

The feature is experimental. The current constants in ``warp/native/apic_types.h`` define writer version 17 and a readable range of versions 13 through 17. Version 17 makes each module's binary kind, target architecture, and architecture suffix format-neutral authoritative metadata. Version 16 changes kernel launch shapes from signed to unsigned 32-bit extents; readers reject high-bit launch shapes from older formats because their intended values are ambiguous. These numbers describe the operation and metadata wire format, not the Python package version.
