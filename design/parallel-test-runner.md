...
The runner is validated by a dedicated harness under `tools/ci/tests/`, kept
outside `warp/tests/**` and the default suite on purpose: the repo rule bars
test-harness meta-tests there. Run it with unittest discovery:

```bash
uv run --extra dev -m unittest discover -s tools/ci/tests -t . -p "test_unittest_parallel_*.py"
```
