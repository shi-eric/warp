...
_SMEM_DOC_URL = "https://nvidia.github.io/warp/stable/user_guide/programming_model/tiles.html#tile-shared-memory-budget"

_SMEM_MITIGATION = (
    "Warp charges shared memory for every tile expression in the kernel, including tiles that are no longer "
    f"live. Reduce the number of tile expressions, shrink the tiles, or lower block_dim. See {_SMEM_DOC_URL}"
)
...
class KernelHooks:
    def __init__(
        ...
        forward_smem_error: str | None = None,
        backward_smem_error: str | None = None,
    ):
        ...
        self.forward_smem_error = forward_smem_error
        self.backward_smem_error = backward_smem_error
...
class ModuleExec:
    ...
    def get_kernel_hooks(self, kernel) -> KernelHooks:
        ...
        if self.device.is_cuda:
            ...
            forward_smem_error = configure_smem(forward_kernel, "forward", forward_smem_bytes)
            backward_smem_error = (
            ...
            hooks = KernelHooks(
                ...
                forward_smem_error=forward_smem_error,
                backward_smem_error=backward_smem_error,
            )
...
def _raise_cuda_launch_error(kernel: Kernel, device: Device, hooks: KernelHooks, adjoint: bool) -> None:
    ...
    shortfall = hooks.backward_smem_error if adjoint else hooks.forward_smem_error
    if shortfall is not None:
        raise RuntimeError(
            ...
            f"memory, {shortfall}. {_SMEM_MITIGATION}"
        )
...
def get_suggested_block_size(kernel, device: DeviceLike = None) -> tuple[int, int]:
    ...
    if not success:
        if hooks.forward_smem_error is not None:
            raise RuntimeError(
                ...
                f"{hooks.forward_smem_error}. {_SMEM_MITIGATION}"
            )
