...
class Module:
    ...
    @synchronized(_codegen_lock)
    def _run_codegen(self, options: dict, is_cpu: bool) -> tuple[str, str, dict, list, list, bool]:
        """Run the Python-side codegen window.

        Returns ``(source, ext, meta, ltoirs, fatbins, uses_nvshmem)``: the
        emitted C++/CUDA source, its file extension, the metadata dict, snapshots
        of the builder's LTO-IR and fatbin collections, and whether the source
        uses NVSHMEM builtins.
    ...
    def _compile(
        ...
        source_str, source_code_ext, meta, ltoir_values, fatbin_values, uses_nvshmem = self._run_codegen(
            options, is_cpu
        )
