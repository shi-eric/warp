...
class RayTracedRenderer:
    ...
    def render_points(self, name="points", points=None, radius=0.1, colors=None, color=(0.5, 0.5, 0.5),
        ...
        Args:
            ...
            points: (N, 3) particle positions (numpy or wp.array).
            radius: float or (N,) array of per-particle radii.
            colors: (N, 3) per-particle RGB colors, or single (r, g, b) tuple.
            color: Default color if colors is a single tuple.
            materials: (N,) int array. 0=opaque (default), 1=glass.
            glass_ior: Index of refraction for glass material (default 1.5).
            glass_tint: (r, g, b) transmission tint for glass.
            positions: Alias for points (deprecated).
        """
        # Support both 'points' and 'positions' argument names
        if points is None and positions is not None:
            points = positions
        if points is None:
            return
        if isinstance(points, np.ndarray):
            pos_wp = wp.array(points.astype(np.float32), dtype=wp.vec3)
        else:
            pos_wp = points
        ...
        if colors is not None:
            if isinstance(colors, np.ndarray) and colors.ndim == 2:
                ...
                # Single color tuple → expand to uniform array
                color = colors
                colors = None  # Fall through to uniform path below
            else:
                colors = None  # Unrecognized format, use default
    ...
    def render_mesh(self, name="mesh", points=None, indices=None, colors=None, color=(0.5, 0.5, 0.5),
                     # Legacy positional support
                     vertices=None, **kwargs):
    ...
    def render_sphere(self, name="sphere", pos=(0, 0, 0), rot=(0, 0, 0, 1), radius=1.0, color=(0.5, 0.5, 0.5), **kwargs):
        """Render a single sphere (convenience wrapper around render_points)."""
        p = np.array([[float(pos[0]), float(pos[1]), float(pos[2])]], dtype=np.float32)
        self.render_points(name=name, points=p, radius=float(radius), color=color)

    def save(self):
        """No-op for API compatibility with UsdRenderer."""
        pass
