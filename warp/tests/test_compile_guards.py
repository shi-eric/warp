...
class TestCompileGuards(unittest.TestCase):
    def test_add_builtin_requires_compile_guard(self):
        """Omitting compile_guard raises TypeError."""
        with self.assertRaises(TypeError):
            add_builtin("_test_no_guard", input_types={}, value_type=int)
    ...
    # ------------------------------------------------------------------
    # require_guard() — empty string is no-op
    # ------------------------------------------------------------------

    def test_require_guard_empty_is_noop(self):
        """require_guard('') must not add anything (COMPILE_GUARD_ALWAYS)."""
        builder = self._make_builder()
        builder.require_guard("")
        self.assertEqual(builder.required_guards, set())
