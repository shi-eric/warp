...
def test_tape_record_func_jacobi(test, device, capture=False):
    ...
    # Float32 tolerances were calibrated against these float64 references with
    # seeds 0-63 and 123 on x86-64 and CUDA sm_120 (direct and graph replay).
    # Maximum fractions of the allowed error were 57% for forward values and
    # 44% for each gradient. RHS gradients accumulate up to 200 contributions,
    # requiring looser tolerances than the averaged initial-state gradients.
    identity = np.eye(n)
