...
The solver reuses two scratch grids for every pressure solve and backward
callback. Apart from those scratch grids, it stores only timestep-boundary
pressures within a checkpoint segment, so pressure storage is
...
@wp.kernel(enable_backward=False)
def accumulate_pressure_grad(g: wp.array2d[float], pressure_grad: wp.array2d[float]):
    i, j = wp.tid()
    pressure_grad[i, j] += g[i, j]
...
class JacobiSolver:
    ...
    def solve(
        ...
        if tape is not None:
            ...
            tape.record_func(backward, arrays=[div, p0, p1])
