...
class Example:
    ...
    def step(self, step_index) -> None:
        ...
        # NOTE: Use previous step's final pressure as the initial guess
        ...
        # NB: output_index should be self.pressure_iterations*step_index at this point
        wp.launch(
            update_velocities,
            (N_GRID, N_GRID),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_iters", type=int, default=100, help="Total number of training iterations.")
    parser.add_argument(
        "--headless",
        action="store_true",
    ...
    with wp.ScopedDevice(args.device):
        ...
        if not args.headless:
            ...
            fig, ax = plt.subplots()
            image = ax.imshow(example.density_arrays[-1].numpy(), cmap="viridis", origin="lower")
            ax.set_xticks([])
            ax.set_yticks([])
