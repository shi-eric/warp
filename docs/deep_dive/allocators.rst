...
:class:`warp.Allocator` protocol — any object with ``allocate(size_in_bytes)`` and
``deallocate(ptr, size_in_bytes)`` methods.

Setting a Custom Allocator
~~~~~~~~~~~~~~~~~~~~~~~~~~

Use :func:`set_allocator` to set a custom allocator on all CUDA devices, or
:func:`set_device_allocator` for a specific device:

.. code:: python

    wp.set_allocator(my_allocator)           # all CUDA devices
...
.. warning::
    ...
    Custom allocators that do not support stream-ordered allocation may not work
